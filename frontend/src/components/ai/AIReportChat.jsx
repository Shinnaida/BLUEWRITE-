...
import { AlertTriangle, CheckCircle2, ClipboardCheck, FileText, LoaderCircle, PenLine, Send, ShieldCheck, Sparkles, X } from 'lucide-react';
...
function AIReportChat({ reportId, reportStatus = 'Draft', reportContext = {}, onInsertSuggestion, onRegisterApply, onEnsureReportId }) {
  ...
  // In-preview edits: the generated narrative and §I–VI fields are editable
  // BEFORE "Use Report Draft" is clicked. Edits land here and flow through the
  // same apply/insert paths as the untouched draft — no separate untracked copy.
  const [draftEdits, setDraftEdits] = React.useState({});
  const rawNotesSnapshot = React.useRef('');
  React.useEffect(() => { setDraftEdits({}); }, [preview?.action, preview?.suggestion]);

  // The effective draft = AI output with any in-preview officer edits applied.
  const effectiveDraft = React.useMemo(() => {
    if (preview?.action !== 'generate' || !preview.reportDraft) return preview?.reportDraft || null;
    const merged = { ...preview.reportDraft };
    if (preview.action === 'improve') return merged;
    for (const [key, value] of Object.entries(draftEdits)) {
      if (typeof value === 'string') merged[key] = value;
    }
    if (typeof draftEdits.narrative === 'string') merged.narrative = draftEdits.narrative;
    return merged;
  }, [preview, draftEdits]);
  const effectiveSuggestion = typeof draftEdits.narrative === 'string' && preview?.action === 'generate' ? draftEdits.narrative : preview?.suggestion;
  const isPersistedDraft = Boolean(reportId) && reportStatus === 'Draft';
  // Pages that create reports (Create flow) can persist the draft on demand:
  // when provided, AI actions stay clickable before a report exists and the
  // callback creates the Draft record first so ownership checks hold.
  const aiReady = isPersistedDraft || Boolean(onEnsureReportId);

  // Expose the currently pending validated draft to the parent page so that
  // Save as Draft / Submit Report can include it when the Officer saves without
  // explicitly clicking "Use Report Draft". Same safety rules as the button:
  // blocked drafts (reviewReady === false) are never applied, and flagged
  // factual warnings must be acknowledged first.
  React.useEffect(() => {
    onRegisterApply?.(() => {
      if (!preview || !['generate', 'improve'].includes(preview.action)) return null;
      if (preview.reviewReady === false) return null;
      if (preview.factAnalysis?.hasWarning && !warningAcknowledged) return null;
      return preview.action === 'generate' ? effectiveDraft : { narrative: effectiveSuggestion };
    });
    return () => onRegisterApply?.(null);
  }, [preview, warningAcknowledged, onRegisterApply, effectiveDraft, effectiveSuggestion]);

  const request = async (action, { reuseSnapshot = false } = {}) => {
    if (activeAction) return;
    let effectiveReportId = reportId;
    if (!isPersistedDraft && onEnsureReportId) {
      effectiveReportId = await onEnsureReportId();
      if (!effectiveReportId) return; // parent already surfaced the error
    }
    ...
    try {
      const response = await requestReportAssistance({ action, reportId: effectiveReportId, reportData: allowedReportData(sourceContext), writingInstruction: writingInstruction.trim(), presetKeys });
      ...
      setError(requestError.response?.data?.message || (timedOut ? 'The AI service did not respond before the request deadline. Please try again; your entered incident information has not been lost.' : 'Unable to generate the report. Your entered incident information has not been lost.'));
  ...
  const useSuggestion = async () => {
    ...
    try {
      ...
      onInsertSuggestion?.(preview.action === 'generate' ? effectiveDraft : { narrative: effectiveSuggestion });
  ...
  return (
    <section className="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm" aria-label="BLUEWRITE AI Writing Assistant">
      ...
      <div className="space-y-4 p-5" aria-live="polite">
        {!reportId && !onEnsureReportId && <p className="rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm text-blue-900">Save this report as a Draft first. AI requests require a persisted report so BLUEWRITE can verify ownership.</p>}
        ...
        <div className="grid gap-2.5 sm:grid-cols-3">
          {ACTIONS.map(({ key, label, icon: Icon }) => <Button key={key} variant="secondary" className="w-full justify-center" disabled={!aiReady || Boolean(activeAction)} onClick={() => request(key)}>{activeAction === key && <LoaderCircle size={15} className="mr-1.5 animate-spin" aria-hidden="true" />}<Icon size={16} className="mr-2" />{activeAction === key ? (key === 'check' ? 'Checking narrative...' : 'Generating report draft...') : label}</Button>)}
        </div>
        {activeAction && <AIGeneratingPanel action={activeAction} />}
        ...
        {preview && <div className="rounded-xl border border-police-blue-200 bg-blue-50/50 p-4">
          ...
          {preview.action==='generate'&&preview.reportDraft&&<div className="mt-3 rounded-lg border border-blue-200 bg-white p-3 text-sm text-slate-800"><p className="mb-2 text-xs font-bold uppercase tracking-wide text-slate-500">Proposed Report Fields — editable before applying</p><dl className="grid gap-x-4 gap-y-2 sm:grid-cols-[9rem_1fr]">{[['Title','title'],['Incident Type','incident_type'],['Incident Date','incident_date'],['Incident Time','incident_time'],['Location','location'],['Summary','summary'],['Complainant','complainant'],['Victim','victim'],['Suspect','suspect'],['Witness','witness']].map(([label,key])=>{
            const isProse = key === 'summary';
            const value = draftEdits[key] ?? preview.reportDraft[key] ?? '';
            return <React.Fragment key={key}><dt className="pt-1.5 font-semibold text-slate-600">{label}</dt><dd>{isProse
              ? <textarea value={value} onChange={(e)=>setDraftEdits((current)=>({ ...current, [key]: e.target.value }))} rows={2} className="w-full rounded-lg border border-slate-300 bg-white px-2.5 py-1.5 text-sm text-slate-800 focus:border-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-600/20" aria-label={`Edit proposed ${label}`} />
              : <span className="break-words">{preview.reportDraft[key]||'Not supplied — left blank'}</span>}</dd></React.Fragment>;
          })}</dl></div>}
          ...
          <p className="mt-3 text-xs leading-5 text-slate-500">AI-generated draft. Every generated field is editable right here before applying — your edits replace the AI text. Using a report draft updates editable form fields only; it never saves or submits automatically. While this preview is open, a validated draft is included automatically when you Save as Draft or Submit Report.</p>
        </div>}
      </div>
    </section>
  );
